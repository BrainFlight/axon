...
class VectorDBClient:
    ...
    def search_vectors(
        self, vector: List[float], top_k: int = 5, with_payload: bool = True
    ) -> List:
        """
    ...
    def search_vector_by_id(
        self, vector_id: uuid.UUID | str, top_k: int, with_payload: bool = True
    ) -> List:
        """
    ...
    def upsert_vectors(self, vectors: List[VectorInput]):
        ...
        for i in vectors:
            points_list.append(
                PointStruct(id=str(uuid.uuid4()), payload=i.payload, vector=i.vector)
            )

        return self.client.upsert(
            collection_name=self.collection_name, wait=True, points=points_list
        )
